...
//Implement the function withdraw(amount) that returns the minimal number of 
//100, 50 and 20 Naira bills the ATM needs to dispense (in that order) in an array.
...
function withdraw(amount) {
    let bills = [0, 0, 0]; // Initialize array for 100, 50, 20 Naira bills
    bills[0] = Math.floor(amount / 100); // Number of 100 Naira bills
    amount = amount % 100; // Update remaining amount
    bills[1] = Math.floor(amount / 50); // Number of 50 Naira bills
    amount = amount % 50; // Update remaining amount
    bills[2] = Math.floor(amount / 20); // Number of 20 Naira bills
    return bills;
  }
